...
\begin{define}
  Definiamo una \textbf{successione} $\{a_n\}$ una funzione $a\colon \mathbb{N} \to \mathbb{R}$.
\end{define}

\begin{define}
  Diciamo che una successione $a_n$ è \textbf{positiva} se $a_n \geqslant 0~\forall n$
\end{define}

\begin{define}
  Diciamo che una successione $a_n$ è \textbf{limitata} se:
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot [name path=upper, very thick, dashed, color=teal] {2};
    \addplot [name path=lower, very thick, dashed, color=teal] {0};
    ...
    % Labels
    \node[teal, anchor=west] at (axis cs:20,2) {$M$};
    \node[teal, anchor=west] at (axis cs:20,0) {$M$};
    \node[blue, anchor=west] at (axis cs:15,1.5) {$(a_n)$};
...
\begin{define}
  Diciamo che una proposizione $P(n)$, con $n \in \mathbb{N}$, è vera \textbf{definitivamente} se è vera $\forall n \geqslant n_0$, con $n_0 \in \mathbb{N}$.
\end{define}


\begin{define}
  Data una successione $a_n$, diciamo che $a_n$ \textbf{converge}, oppure tende a, oppure ha limite $l \in \mathbb{R}$ se $\exists l \in \mathbb{R}$ t.che:
  $$
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot [very thick, dashed, color=teal] { \L + \eps };
    \addplot [very thick, dashed, color=teal] { \L - \eps };
    ...
    % Labels
    \node[blue, anchor=west] at (axis cs:10,1.6) {$(a_n)$};
...
\begin{thm}[Primo teorema del confronto]
...
\begin{thm}[Teorema di Bolzano-Weierstrass]
...
\begin{proof}
  ...
  \begin{tikzpicture}[scale=3.5, line cap=round, line join=round]
    % Colors
    \definecolor{sector}{HTML}{B3E5FC}
    \definecolor{triangle1}{HTML}{C8E6C9}
    \definecolor{triangle2}{HTML}{FFCDD2}
    ...
    \fill[triangle1!70!white] (O) -- (A) -- (B) -- cycle;

    % Sector OAB
    \fill[sector, opacity=0.5] (0,0) -- (B) arc(0:30:1) -- cycle;

    % Large triangle OAC (tan x)
    \fill[triangle2!70!white, opacity=0.6] (O) -- (A) -- (C) -- cycle;
    ...
    % Angle marker
    \draw[->, thick, teal] (0.3,0) arc(0:30:0.3);
    \node at (0.38,0.08) {$x$};
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    % Limit line
    \addplot [very thick, teal, dashed, domain=0:20] {1.5} node [pos=1.01, right] {$L$};

    % ε-tube around the limit
    \addplot [name path=upper, draw=none, domain=0:20] {1.5 + 0.3};
    \addplot [name path=lower, draw=none, domain=0:20] {1.5 - 0.3};
    ...
    % Label ε tube
    % \draw[decorate, decoration={brace, mirror, amplitude=5pt}, thick, teal]
    %   (axis cs:20,1.8) -- (axis cs:20,1.2)
    %   node[midway,xshift=1.3cm,teal] {$2\varepsilon$};

    % N marker
    \draw[dashed, gray] (axis cs:9,0) -- (axis cs:9,2.5);
    \node[gray, below] at (axis cs:9,0) {$n_0$};
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    \addplot[mark=*, blue, domain=2:30, samples=20, thick] {(1 + 1/x)^x};

    % Plot for b_n (decreasing)
    \addplot[mark=*, teal, domain=2:30, samples=20, thick] {(1 + 1/x)^(x+1)};
    
    \node[anchor=north, blue!70!black] at (axis cs:5, {(1 + 1/5)^5 + 0.15}) {$a_n$};
    \node[anchor=south, teal!70!black] at (axis cs:5, {(1 + 1/5)^6 - 0.15}) {$b_n$};
