...
\begin{center}
  ...
  \addplot[domain=0:5.5, smooth, very thick, cmapteal-300]
      {e^(sin(deg(x)))*sqrt(x)};

  \addplot[domain=0.5:3, smooth, thick, amber]
      {((e^(sin(deg(2)))*sqrt(2) - e^(sin(deg(1))))/(1))*(x - 1) + e^(sin(deg(1)))};

  % Coordinates
  \coordinate (X) at (1, {e^(sin(deg(1)))*sqrt(1)});
  \coordinate (Y) at (2, {e^(sin(deg(2)))*sqrt(2)}); % z_lambda at center

  % Mark points
  \addplot[only marks, mark=*] coordinates {(1, {e^(sin(deg(1)))*sqrt(1)})};
  \addplot[only marks, mark=*] coordinates {(2, {e^(sin(deg(2)))*sqrt(2)})};

  % Labels
  \node[below] at (axis cs:1,0) {$x_0$};
  \node[below] at (axis cs:2,0) {$x_0 + h$};
...
\begin{defineimp}[Differenziabilità]
...
\begin{prop}[Derivata della somma]
  Siano $f$ e $g$ due funzioni derivabili in $x_0$, la loro somma è derivabile in $x_0$ e:
...
\begin{prop}[Derivata del prodotto]
  Siano $f$ e $g$ due funzioni derivabili in $x_0$, il loro prodotto è derivabile in $x_0$ e:
...
\begin{prop}[Derivata della composta]
  Sia $f: \mathfrak{D}(f) \to \mathfrak{D}(g)$ una funzione derivabile in $x_0$ e $g: \mathfrak{D}(g) \to \mathbb{R}$ 
  una funzione derivabile in $f(x_0)$. Allora la funzione composta $\eta := g \circ f$ è derivabile in $x_0$ e:
  
  $$
    \eta'(x_0) = g'(f(x_0))f(x_0)
  $$
\end{prop}

\begin{proof}
  Si utilizzerà l'equivalente nozione di differenziabilità al posto di quella di derivabilità. Si ha dunque che:

  $$
    f(x_0 + h) = f(x_0) + f'(x_0)h + h \omega_f(h)
  $$
  $$
    g(y_0 + k) = g(y_0) + g'(y_0)k + k \omega_g(k)
  $$

  di conseguenza:

  \begin{align*}
    \eta(x_0 + h) = g(f(x_0 + h)) &= g(\equalto{f(x_0)}{y_0} + \underset{k \to 0}{\underbrace{f'(x_0)h + h \omega_f(h)}}) = \\
                                  &= g(y_0) + g'(y_0)\left[f'(x_0)h + h\omega_f(h)\right] + \left[f'(x_0)h + h\omega_f(h)\right]\omega_g(k)
  \end{align*}

  $$
    \eta(x_0 + h) = g(y_0) + g'(y_0)f'(x_0)h + h\omega(h) = \eta(x_0) + \eta'(x_0)h + h\omega(h)
  $$
\end{proof}


\begin{prop}[Derivata dell'inversa]
  Sia $f: (a, b) \to \mathbb{R}$ una funzione continua e strettamente monotona. Se $f$ è derivabile in $x_0 \in (a, b)$ e 
  $f'(x_0) \neq 0$ allora $f^{-1}$ è derivabile in $y_0 = f(x_0)$ e:
...
\begin{proof}
  ...
  $$
    R\left(f^{-1}, y_0, k\right) = \dfrac{f^{-1}(y_0 + k) - f^{-1}(y_0)}{k}
  $$

  Sia $h = f^{-1}(y_0 + k) - f^{-1}(y_0)$. \\
  Dato che $f^{-1}$ è continua, $h \underset{k \to 0}{\to} 0$. Quindi:
  ...
  $$
    f(x_0 + h) = y_0 + k = f(x_0) + k \implies k = f(x_0 + h) - f(x_0)
  $$

  quindi:

  $$
    R\left(f^{-1}, y_0, k\right) = \dfrac{h}{f(x_0 + h) - f(x_0)} = \dfrac{1}{\dfrac{f(x_0 + h) - f(x_0)}{h}} \underset{k \to 0}{\to} \dfrac{1}{f'(x_0)}
  $$
\end{proof}
...
\begin{define}[Massimo locale]
  Sia $f: \mathfrak{D}(f) \to \mathbb{R}$ una funzione e sia $x_0 \in \mathfrak{D}(f)$. $x_0$ si dice 
  \textbf{massimo locale} di $f$ se $\exists \mathcal{U}(x_0)$ t.che:

  $$
    f(x) \leqslant f(x_0)~\forall x \in \mathcal{U}(x_0) \cap \mathfrak{D}(f)
  $$
\end{define}

\begin{define}[Minimo locale]
  Sia $f: \mathfrak{D}(f) \to \mathbb{R}$ una funzione e sia $x_0 \in \mathfrak{D}(f)$. $x_0$ si dice 
  \textbf{minimo locale} di $f$ se $\exists \mathcal{U}(x_0)$ t.che:

  $$
    f(x) \geqslant f(x_0)~\forall x \in \mathcal{U}(x_0) \cap \mathfrak{D}(f)
  $$
\end{define}

Se $x_0$ è un minimo o un massimo locale, si dice \textbf{estremante locale}.


\begin{thm}[Teorema di Fermat]
  Sia $f: \mathfrak{D}(f) \to \mathbb{R}$ una funzione e sia $x_0 \in \mathfrak{D}(f)$ un estremante locale di $f$. Se $f$ 
  è derivabile in $x_0$, si ha che:
...
\begin{proof}
  Si supponga che $x_0$ sia un massimo locale. Per definizione, esiste un intorno $\mathcal{U}(x_0) = (x_0 - h, x_0 + h)$, con $h > 0$, t.che:
  
  $$
    f(x) \leqslant f(x_0)~\forall x \in (x_0 - h, x_0 + h)
  $$

  dato che $f$ è derivabile in $x_0$:

  $$
    f'(x_0) = L \in \mathbb{R} \implies f'_-(x_0) = f'_+(x_0) = f'(x_0)
  $$

  ma si ha che, per il teorema della permanenza del segno:

  $$
    f'_-(x_0) = \lim_{h \to 0^-}\dfrac{f(x_0 + h) - f(x_0)}{h} \geqslant 0
  $$
  $$
    f'_+(x_0) = \lim_{h \to 0^+}\dfrac{f(x_0 + h) - f(x_0)}{h} \leqslant 0
  $$
...
\begin{thmimp}[Teorema di Lagrange]
...
\begin{center}
  ...
  \addplot[domain=0.5:5.5, smooth, thick, cmapteal-500]
  ...
  % Coordinates
  \coordinate (X) at (1, {e^(cos(deg(1)))*sqrt(1)});
  \coordinate (Y) at (5, {e^(cos(deg(5)))*sqrt(5)});
  \coordinate (C) at (3.44, {e^(cos(deg(3.44)))*sqrt(3.44)});

  % Mark points
  \addplot[only marks, mark=*] coordinates {(1, {e^(cos(deg(1)))*sqrt(1)})};
  \addplot[only marks, mark=*] coordinates {(5, {e^(cos(deg(5)))*sqrt(5)})};
  \addplot[only marks, mark=*] coordinates {(3.44, {e^(cos(deg(3.44)))*sqrt(3.44)})};
...
\begin{proof}
  ...
  si ha che $h(x)$ soddisfa le ipotesi di Rolle. Quindi:

  $$
    \exists c \in (a, b)~\colon~f'(c)[g(b) - g(a)] - g'(c)[f(b) - f(a)] = 0
  $$
\end{proof}


\begin{thm}
  Sia $f \in \mathcal{C}'(\mathcal{U}(a))$, con $a \in \mathbb{R}$ t.che $f'(a) \neq 0$. Siano $a_n$ e $b_n$ due 
  successioni t.che $a_n \neq b_n$, $a_n \to a$, $b_n \to b$, allora:

  $$
    f(a_n) - f(b_n) \sim f'(a)(a_n - b_n)
  $$
\end{thm}
...
\begin{proof}
  ...
  quindi, dato che $f$ è differenziabile:

  $$
    f(a_n) = f(a + \varepsilon_n) = f(a) + f'(a)\varepsilon_n + \varepsilon_n \omega(\varepsilon_n)
  $$

  $$
    f(a_n) - f(a) = f'(a)\varepsilon_n + \varepsilon_n \omega(\varepsilon_n) \sim f'(a)(a_n - a)
  $$
\end{proof}



\begin{thmimp}[Condizione sufficiente di derivabilità]
...
\begin{center}
  ...
  % Convex function (example)
  \addplot[domain=0.5:5.5, smooth, very thick, cmapteal-300]
      {0.5*(x-3)^2 + 1};
  ...
  % Line segments r and p
  \draw[cmapteal-500, thick] (X) -- (Z) node[midway, above] {$r$};
  \draw[cmapteal-500, thick] (Z) -- (Y) node[midway, above] {$p$};
...
\begin{center}
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[color=cmapteal-300, very thick]{x*ln(x)};

      \addplot[color=cmapteal-500]{x-1};

      \coordinate (X) at (1, 0);
      \coordinate (Y) at (2, {2*ln(2)});
      \coordinate (Z) at (1.5, {1.5*ln(1.5)});

      \draw[dashed, gray] (1.5, 0) -- (Z);
      \draw[dashed, gray] (2, 0) -- (Y);

      % Mark points
      \addplot[only marks, mark=*] coordinates {(1, 0)};
      \addplot[only marks, mark=*] coordinates {(2, {2*ln(2)})};
      \addplot[only marks, mark=*] coordinates {(1.5, {1.5*ln(1.5)})};

      % Labels
      \node[below] at (axis cs:1,-0.05) {$x_0$};
      \node[below] at (axis cs:2,0) {$t$};
      \node[below] at (axis cs:1.5,0) {$x_0 + h$};
...
\begin{proof}
  ...
  $$
    \tau(x) = f'(x_0)(x - x_0) + f(x_0)
  $$

  sia $h > 0$ e sia $t > x_0 + h$. Si ha che:

  $$
    x_0 + h = \lambda t + (1 - \lambda)x_0 = x_0 + \lambda(t - x_0) \implies h = \lambda(t - x_0)
  $$

  dato che $f$ è convessa:

  $$
    f(x_0 + h) \leqslant \lambda f(t) + (1 - \lambda)f(x_0) \implies f(x_0 + h) - f(x_0) \leqslant \lambda[f(t) - f(x_0)]
  $$

  dato che:

  $$
    \lambda = \dfrac{h}{t - x_0}
  $$

  si ottiene:

  $$
    f(x_0 + h) - f(x_0) \leqslant \dfrac{h}{t - x_0}[f(t) - f(x_0)] \implies \dfrac{f(x_0 + h) - f(x_0)}{h} \leqslant \dfrac{f(t) - f(x_0)}{t-x_0}
  $$

  per il teorema della permanenza del segno, per $h \to 0$ si ha:

  $$
    f'(x_0) \leqslant \dfrac{f(t) - f(x_0)}{t-x_0} \implies \tau(t) \leqslant f(t)
  $$
\end{proof}
...
\begin{thm}
  Sia $f: [a, b] \to \mathbb{R}$ t.che $f$ è derivabile in $[a, b]$ e $\forall x_0 \in [a, b]$ $f(x) \geqslant \tau_{x_0}(x)~~~\forall x \in [a, b]$, dove $\tau_{x_0}$ è la 
  tangente al grafico di $f$ in $x_0$ $\implies$ $f$ è convessa in $[a, b]$.
\end{thm}

\begin{proof}
  Sia $x_0 \in [a, b]$ fissato, si ha che:

  $$
    \tau_{x_0}(t) = f'(x_0)(t - x_0) + f(x_0)
  $$

  siano $x, y \in [a, b]$ t.che $x < x_0 < y$. Si ha che:
  ...
  $$
    \begin{cases*}
      f(y) \geqslant \tau_{x_0}(y) \implies f(y) \geqslant f'(x_0)(y - x_0) + f(x_0) \\
      f(x) \geqslant \tau_{x_0}(x) \implies f(x) \geqslant f'(x_0)(x - x_0) + f(x_0)
    \end{cases*}
  $$

  $$
    y - x_0 = y - \lambda x - y + \lambda y = \lambda (y - x)
  $$

  $$
    x - x_0 = x - \lambda x - y + \lambda y = (x - y)(1 - \lambda)
  $$
  ...
  $$
    \begin{cases*}
      f(y) \geqslant f'(x_0)\lambda (y - x) + f(x_0) \\
      f(x) \geqslant f'(x_0)(x - y)(1 - \lambda) + f(x_0)
    \end{cases*}
  $$

  $$
    \begin{cases*}
      (1 - \lambda)f(y) \geqslant (1 - \lambda)f'(x_0)\lambda (y - x) + (1 - \lambda)f(x_0) \\
      \lambda f(x) \geqslant -\lambda f'(x_0)(y - x)(1 - \lambda) + \lambda f(x_0)
    \end{cases*}
  $$

  \begin{align*}
    \lambda f(x) + (1 - \lambda)f(y)  &\geqslant (1 - \lambda)f'(x_0)\lambda (y - x) - \lambda f'(x_0)(y - x)(1 - \lambda) + f(x_0) = \\
                                      &= \cancel{\lambda f'(x_0)(y - x)} - \cancel{\lambda^2 f'(x_0)(y - x)} - \cancel{\lambda f'(x_0)(y - x)} + \cancel{\lambda^2f'(x_0)(y - x)} + f(x_0) = \\
...
\begin{proof}\leavevmode
  \begin{itemize}
    \item ($\Leftarrow$) Si vuole mostrare che $f$ sta sopra ogni tangente. Sia $x_0$ fissato, si vuole mostrare che:
          $$
            f(x) \geqslant \tau_{x_0}(x)~\forall x > x_0
          $$

          dove $\tau_{x_0}$ è la tangente al grafico di $f$ in $x_0$. Si applichi allora il teorema di Lagrange all'intervallo $[x, x_0]$:

          $$
            \exists y \in (x, x_0)~\colon~f(x) - f(x_0) = f'(y)(x - x_0)
          $$

          dato che $f'$ è crescente, $f'(y) \geqslant f'(x_0)$. Quindi:

          $$
            f(x) - f(x_0) \geqslant f'(x_0) f'(x_0)(x - x_0) \implies f(x) \geqslant \tau_{x_0}(x)
          $$
  \end{itemize}
\end{proof}
...
\begin{proof}
  La dimostrazione segue dal teorema precedente, dato che:

  $$
    f'~~~\text{ crescente }~\iff f''(x) \geqslant 0~~~\forall x \in [a, b]
  $$
\end{proof}
...
Sia $f: \mathfrak{D}(f) \to \mathbb{R}$ una funzione $n$ volte derivabile e sia $x_0 \in \mathfrak{D}(f)$. Si vuole trovare 
un polinomio $p_n$ di grado $n$ che "approssimi" la funzione $f$ in $x_0$. Per approssimare si intende che le prime $n$ derivate di $p_n$ uguagliano 
le prime $n$ derivate di $f$ in $x_0$. Sia quindi $p_n$ un generico polinomio di grado $n$:

$$
  p_n(x) = a_0 + a_1(x-x_0) + a_2(x-x_0)^2 + \dots + a_n(x-x_0)^n
$$

le cui derivate risultano:

\begin{align*}
  p'_n(x)       &= a_1 + 2a_2(x-x_0) + 3a_3(x-x_0)^2 + \dots + n a_n (x-x_0)^{n-1} \\
  p''_n(x)      &= 2a_2 + 6a_3(x-x_0) + \dots + n(n-1)(x-x_0)^{n-2} \\
  p'''_n(x)     &= 6a_3 + \dots + n(n-1)(n-2)(x-x_0)^{n-3} \\
  p^{(k)}_n(x)  &= k! \cdot a_k + \dots + \dfrac{n!}{(n-k)!} (x-x_0)^{n-k} \\
  p^{(n)}_n(x)  &= n! \cdot a_n
\end{align*}

quindi i coefficienti di $p_n$ possono essere determinati come (con $k < n$):

$$
  \begin{cases*}
    p_n(x_0)        = f(x_0)          \implies     f(x_0)         = a_0           \implies a_0 = f(x_0) \\
    p'_n(x_0)       = f'(x_0)         \implies     f'(x_0)        = a_1           \implies a_1 = f'(x_0) \\ 
    p''_n(x_0)      = f''(x_0)        \implies     f''(x_0)       = 2a_2          \implies a_2 = \dfrac{f''(x_0)}{2} \\
    p'''_n(x_0)     = f'''(x_0)       \implies     f'''(x_0)      = 6a_3          \implies a_3 = \dfrac{f'''(x_0)}{6} \\
    p^{(k)}_n(x_0)  = f^{(k)}(x_0)    \implies     f^{(k)}(x_0)   = k! \cdot a_k  \implies a_k = \dfrac{f^{(k)(x_0)}}{k!} \\
    p^{(n)}_n(x_0)  = f^{(n)}(x_0)    \implies     f^{(n)}(x_0)   = n! \cdot a_n  \implies a_n = \dfrac{f^{(n)}(x_0)}{n!}
  \end{cases*}
$$
...
\begin{thmimp}[Teorema di Taylor-Peano]
...
\begin{thm}[Teorema di Taylor-Lagrange]
  Sia $x_0 \in \mathfrak{D}(f)$ fissato e sia $f$ una funzione derivabile $n-1$ volte in $[x_0, x]$ (o $[x, x_0]$) e sia 
  $p_n(x)$ il polinomio di Taylor di $f$ di ordine $n$ centrato in $x_0$ $\implies$ $\exists y \in (x_0, x)$ (o $(x, x_0)$) t.che:

  $$
    f(x) = p_n(x) + R_n(x)
  $$

  dove $R_n(x)$ è detto \textbf{resto di Lagrange} ed è definito come:

  $$
    R_n(x) := \dfrac{f^{(n+1)}(y)}{(n+1)!}(x-x_0)^{n+1}
  $$
\end{thm}
...
\begin{thmimp}[Teorema di de l'Hôpital]
...
\begin{proof}
  Si dimostrerà il caso in cui si ottiene la forma indeterminata $\dfrac{0}{0}$ con $x_0 \in \mathbb{R}$. Si considerino le estensioni di $f$ e $g$ in $x_0$ $\hat{f}$, $\hat{g}$. Si ha che:

  $$
    \hat{f}(x_0) = \hat{g}(x_0) = 0
  $$ 

  inoltre, per il teorema di Cauchy, in $\dot{\mathcal{U}}(x_0)$:

  $$
    \dfrac{f(x)}{g(x)} = \dfrac{\hat{f}(x) - \hat{f}(x_0)}{\hat{g}(x) - \hat{g}(x_0)} = \dfrac{f'(y)}{g'(y)} \underset{y \to x_0}{\to} l
  $$

  con $x < y < x_0$ (se $x_0 > x$).
\end{proof}
