...
\begin{define}[Punto di accumulazione]
...
\begin{center}
  ...
  {
    \filldraw[blue] ({1 + \x/3},0) circle (2.2pt);
  }

  \filldraw[amber] (1,0) circle (2pt);
...
\begin{center}
  ...
  \foreach \x in {5, 2.3, 1.4, 0.8, 0.4, 0.15}
  {
    \filldraw[teal] ({1 + \x/3},0) circle (2.2pt);
  }
...
\begin{thm}[Permanenza del segno]
  Sia $f: \mathfrak{D}(f) \to \mathbb{R}$ una funzione reale e sia $x_0$ un punto di accumulazione di $\mathfrak{D}(f)$.
...
\begin{center}
  ...
  \addplot[blue, thick] ({x},{sin(deg(x))/x});
...
\begin{center}
  ...
  \addplot[only marks, mark=o, blue, mark size=3pt] coordinates {(0,-1)};
...
\begin{center}
  ...
  \addplot[blue, thick, domain=0:10] ({x},{1/x});
  \addplot[blue, thick, domain=-10:0] ({x}, 0);
...
\begin{center}
  ...
  \addplot[blue, thick, domain=0.001:2] ({x},{sin(deg(1/x))});
  \addplot[blue, thick, domain=-2:0] ({x}, -x);
  ...
  \addplot[blue, only marks, mark=*] coordinates {(0,2)};
...
\begin{center}
  ...
  \node[dot, blue, label={above right:$(\text{a}, \text{f}(\text{a}))$}] at (\a, \fa) {};
  ...
  \node[dot, blue, label={below right:$(\text{b}, \text{f}(\text{b}))$}] at (\b, \fb) {};
  ...
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[color=blue]{4*e^(-x)*sin(deg(x))*ln(x)};
...
\begin{thm}[Teorema di Weierstrass]
